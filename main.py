    ...
    if isInGap is True:
        ...
        if (len(QueueDB.get(m.from_user.id)) >= 0) and (len(QueueDB.get(m.from_user.id)) <= Config.MAX_VIDEOS):
            ...
            if len(QueueDB.get(m.from_user.id)) == Config.MAX_VIDEOS:
                MessageText = "That's the Limit, Now Just Press **Merge Now** Button Plox!"
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        if Config.UPDATES_CHANNEL:
            try:
                ...
                if user.status == "kicked":
                    await cb.message.edit(
                        text="Sorry Sir, You are Banned to use me. Contact [MasterMind](https://t.me/MasterMind007).",
                        parse_mode="markdown",
                        disable_web_page_preview=True
                    )
                    return
                ...
                await cb.message.edit(
                    text="Due to some Error!!! Something went Wrong.",
                    parse_mode="markdown",
                    disable_web_page_preview=True
                )
                return
        await cb.message.edit(
            text=Config.START_TEXT,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer - @MasterMind007", url="https://t.me/MasterMind007")], [InlineKeyboardButton("Network Channel", url="https://t.me/Mastermind_Network")]]),
            disable_web_page_preview=True
        )
        ...
        try:
            ...
            await cb.answer("Your Queue Empty Sir!", show_alert=True)
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Sir, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/File_Merged.{FormtDB.get(cb.from_user.id).lower()}"
